...
ClickAndEnter(ctrl, winTitle, delay := 200) { ; 200ms 
    log("ClickAndEnter " ctrl " in winTitle: " winTitle)
    ControlFocus, %ctrl%, %winTitle%   ;; Recommanded safe way
    Sleep, %delay%
    SendInput, {Space}
    Sleep, %delay%
...
ClickAndText(ctrl, value, winId, winTitle, delay := 200) {
    ...
    ControlFocus, %ctrl%, %winTitle%     ;;     Working, Recommanded safe way
...
Finish() {
    ...
    log("오늘도 신이 주시는 치유와 평안이 함께 하셨으면 좋겠습니다. ")
    log("-----------------------------------------------------------")
    log("")
    log("")
...
ClickAndEnter(loginClass, mainWinTitle)   ; 로그인 버튼
log("Logged in")
Sleep, 2000

; ----- square and composit wave
ClickAndEnter(squareWaveClass, mainWinTitle)
ClickAndEnter(compositeClass, mainWinTitle)

; ----- pop up the read window -----
ClickAndEnter(ReadPopupClass, mainWinTitle)
...
ClickAndEnter(readWin_saveTimeClass, readWinTitle)
ClickAndEnter(readWin_readBtnClass, readWinTitle)
Sleep, 3000
ClickAndEnter(startClass, mainWinTitle, 1000)
